...
export default defineComponent({
    setup() {
            ...
            (value) => {
                switch (value) {
                    case "detail":
                        ...
                        break;
                    case "attributes":
                        router.push({ name: "DeviceAttributes" });
                        break;
                    case "connection":
                        router.push({ name: "DeviceConnection" });
                        break;
                    case "add":
        ...
        const data = computed(() => {
            return device_store.state.device_list
                .map((dev: IDeviceShort) => {
                    return {
                        id: dev.id,
                        name: dev.name,
                ...
                .filter((dev) => {
                    return dev.name.toLocaleLowerCase().includes(search.value.toLocaleLowerCase()) || dev.type.toLocaleLowerCase().includes(search.value.toLocaleLowerCase());
                });
        });
